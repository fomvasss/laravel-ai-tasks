...
> **Безпека:** дефолтний `middleware => ['web']` лишає дашборд відкритим для будь-кого, хто має доступ до URL — включно зі збереженими промптами і відповідями, **а тепер і з кнопками Retry / Dead нижче**. У production додай свій auth middleware: `['web', 'auth']`, або напр. `['web', 'auth', 'role:admin']` зі spatie/laravel-permission.

### Зависші прогони

Прогін вважається **stuck**, якщо він `queued` або `running` довше за `stuck_after_minutes` без жодного
прогресу. Типова причина — job, яка так і не доїхала до воркера (рестарт Redis між тим, як `AI::queue()`
записав рядок, і моментом, коли воркер мав його забрати): нікому більше позначити прогін помилкою, тож він
лишається `queued` назавжди, і жоден механізм ретраю до нього не дістає.

Такі прогони мають власну картку статистики, фільтр `stuck` і бейдж у рядку. Поріг має бути більшим за
найповільнішу реальну задачу, інакше під нього потраплять і легітимні довгі прогони.

Дві дії — у рядку списку і на сторінці прогону:

- **Retry** — відновлює задачу з `ai_runs.request` і диспатчить наново, перевикористовуючи той самий рядок.
  Доступна для `error`/`dead` і для зависших `queued`/`running` (не зависший `running` не чіпається — над ним
  зараз реально працює воркер). Потребує, щоб при створенні прогону був увімкнений `store_request`, інакше
  немає аргументів конструктора для відновлення.
- **Dead** — закрити прогін, на якому поставили хрест: `status = dead`, причина в `error`. Подію
  `AiRunFailed` **не** фаєрить — реальна невдача сталася раніше й мовчки, і споживачі цієї події не мають
  отримувати сповіщення про натискання кнопки адміном.

Той самий шлях доступний з CLI, включно зі зависшими:
...
**Ставки під конкретну модель.** `price` — це один набір ставок на драйвер, тоді як сама модель береться з `.env`: перехід на дорожчу модель мовчки лишає стару собівартість. Винятки перелічуються в `prices` за іменем моделі, решта падає на `price`:
